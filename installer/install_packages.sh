...
apt -o Dpkg::Options::="--force-confold" --assume-yes install -y sudo resolvconf unbound jq python3-pexpect python3-mysqldb openssh-server mariadb-server nodejs npm nftables kea curl screen vlan irqbalance ethtool samba iperf3 ca-certificates iftop
