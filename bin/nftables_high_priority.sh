...
# (All your existing nftables firewall, NAT, hairpin rules, etc.)
# We keep these EXACTLY as in your script:
...
# Mark all UDP as EF (highest priority)
# (This effectively ensures *any* UDP gets EF, including games)
nft add rule ip mangle postrouting udp ip dscp set ef

# Mark small packets (gaming traffic) with DSCP CS4 (optional, can be removed since we set EF above)
nft add rule ip mangle postrouting ip length 0-128 ip dscp set cs4

# Allow tailscale
nft add rule inet filter input iif "tailscale0" accept

# ================= INGRESS DSCP CLASSIFICATION ======================== #
echo "Adding filters to classify ingress traffic..."
# Priority mapping: EF -> highest priority, then others
# (0x2e = DSCP EF, 0xfc = mask)
tc filter add dev ifb0 parent 1: protocol ip u32 match ip tos 0x2e 0xfc action skbedit priority 3 || { echo "Failed to add filter for EF"; exit 1; }

# AF41 -> slightly lower priority
tc filter add dev ifb0 parent 1: protocol ip u32 match ip tos 0x18 0xfc action skbedit priority 2 || { echo "Failed to add filter for AF41"; exit 1; }

# CS1 -> best effort
tc filter add dev ifb0 parent 1: protocol ip u32 match ip tos 0x08 0xfc action skbedit priority 1 || { echo "Failed to add filter for CS1"; exit 1; }
