...
#INTERFACES=$(ip link show | awk '/^[0-9]+: [^lo]/ {print $2}' | sed 's/:$//')
INTERFACES=$(ip link show | awk '/^[0-9]+: [^lo]/ {iface=$2; if (!match(iface, /^ifb[0-9]+:/)) print iface}' | sed 's/:$//')
...
# Assign static IP to the interface without IP
if [ ${#NO_IP[@]} -eq 1 ]; then
    IFACE=${NO_IP[0]}
    ...
    echo "Error: There are $((${#NO_IP[@]})) interfaces without IP addresses. Expected only one."
fi
